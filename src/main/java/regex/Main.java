...
public class Main {
    ...
    public static boolean checkForPassword(String str, int minLength) {
        ...
        final boolean propertyOne = Pattern.matches(".+", str);
        final boolean propertyTwo = Pattern.compile("[a-z]").matcher(str).find();
        final boolean propertyThree = Pattern.compile("[A-Z]").matcher(str).find();
        final boolean propertyFour = Pattern.compile("[0-9]").matcher(str).find();
        final boolean propertyFive = str.length() >= minLength;
        return propertyOne && propertyTwo && propertyThree && propertyFour && propertyFive;
    }
